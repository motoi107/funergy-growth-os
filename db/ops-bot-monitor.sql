alter table public.bot_cases drop constraint bot_cases_kind_check;
alter table public.bot_cases add constraint bot_cases_kind_check check (kind in ('labor','void','unpaid','purchase'));
alter table public.bot_cases add column last_checked_at timestamptz;
alter table public.bot_cases add column last_check jsonb;
create index bot_monitor_pending on public.bot_cases(store_id,last_checked_at nulls first,created_at) where status<>'done';
...
create function public.bot_record_check(p_id uuid,p_version integer,p_actor uuid,p_result jsonb) returns void
language plpgsql security invoker set search_path=public,pg_temp as $$
...
revoke all on function public.bot_record_check(uuid,integer,uuid,jsonb) from public,anon,authenticated;
grant execute on function public.bot_record_check(uuid,integer,uuid,jsonb) to service_role;
