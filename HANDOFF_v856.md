...
| | 状態 |
|---|---|
| 現行版 | **v856**。**v849〜v856 が GitHub 未 push**（v848 までは push 済み） |
| 検証 | `PASS=4629 / FAIL=0 / 実行=80/95`。検証95本・バックアップ75件 |
...
| **Motoさん待ち③** | **本部予算**（管理者セキュアページ→本部予算タブ）で 客数・客単価。KPI管理ぶんは設定済み |
...
```bash
unzip funergy_v856_handoff.zip
tar -xJf handoff_v856.tar.xz
cd handoff_v856
...
```
合計 PASS=4629  FAIL=0  実行=80/95
合流ルールの未定義: 1
同期対象で未保護: 0
OP_SYNC_KEYS で未保護: 61
予算の対象月: 3 / 7
APP_VERSION=856 / SW_BUILD=856
CRLF=58145 / lone LF=0
```
