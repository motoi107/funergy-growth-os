/* v726 日次食材差異(丼ごと設定版) — 自動テスト（実コードをモック環境で実行） */
const fs = require('fs');
const vm = require('vm');
...
vm.runInContext(fs.readFileSync('/home/claude/fv_module_v726.js', 'utf8'), ctx);
...
console.log('== 20) 丼ごとの設定と「今日から変更」で過去が変わらないこと ==');
{
  const cfg=setupConfig();
  cfg.recipes={};                      /* まっさらから丼単位で登録 */
  store['fv_config']=cfg;
  /* 画面フォーム相当のモック */
  const form={ tuna:'40', negitoro:'', salmon:'', ikura:'', crab:'', uni:'' };
  let mode='today';
  ctx.document.querySelector=(sel)=>{
    const m=sel.match(/\.fvb-g\[data-id="(\w+)"\]/);
    if(m) return { value: form[m[1]] };
    if(sel.indexOf('fvb-mode')>=0) return { value: mode };
    return null;
  };
  ctx.fvBowlSubmit('GUID_AHI');
  let set=ctx.fvBowlSet('GUID_AHI', ctx.fvGetConfig(), '2026-07-25');
  eq('丼にまぐろ40gが登録される', set.tuna, 40);
  const day={ g:{ GUID_AHI:10 }, tq:10, n:1 };
  eq('理論使用量 = 10個×40g = 400g', ctx.fvG(ctx.fvTheoreticalDay('F04-K','2026-07-25',day,ctx.fvGetConfig()).tuna.dg), 400);
  /* 50gへ変更（今日=2026-07-25 から） */
  form.tuna='50';
  ctx.fvBowlSubmit('GUID_AHI');
  eq('今日(7/25)は新しい50g → 500g', ctx.fvG(ctx.fvTheoreticalDay('F04-K','2026-07-25',day,ctx.fvGetConfig()).tuna.dg), 500);
  eq('前日(7/24)は旧40gのまま → 400g【過去は不変】', ctx.fvG(ctx.fvTheoreticalDay('F04-K','2026-07-24',day,ctx.fvGetConfig()).tuna.dg), 400);
  /* 空欄にすると今日以降だけ計上されなくなる */
  form.tuna='';
  ctx.fvBowlSubmit('GUID_AHI');
  eq('空欄 → 今日は0g', ctx.fvG(ctx.fvTheoreticalDay('F04-K','2026-07-25',day,ctx.fvGetConfig()).tuna.dg), 0);
  eq('空欄でも過去(7/24)は400gのまま', ctx.fvG(ctx.fvTheoreticalDay('F04-K','2026-07-24',day,ctx.fvGetConfig()).tuna.dg), 400);
