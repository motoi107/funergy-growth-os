...
```
合計 PASS=3138  FAIL=0  実行=59/74
合流ルールの未定義: 1        ← spl_skills_st_ の1件だけが既知（v807 から変化なし）
同期対象で未保護: 1          ← 残りは死んでいる labor_hours_ だけ（第2章①）
予算の対象月: 3 / 7          ← 要確認3件は確認済みの偽陽性
APP_VERSION=835 / SW_BUILD=835
CRLF=56365 / lone LF=0
```
...
### 合流ルールの穴を塞ぐ — v825、v826

| 版 | 内容 |
|---|---|
| v825 | `daily_actuals_`（日次売上）。事故対応（次項） |
| — | **監査の修正**（製品は変更なし）。`merge_probe.js` を新設し、判定を実 `_opMergeDef` に1本化。対象を `OP_SYNC_PREFIX` 全部へ拡大。第2章② |
| v826 | `fc_monthly_`（原価 月次）。単一オブジェクトなので項目ごとに合流。`setFcMonthly()` に `_at`/`_by` を追加 |
| v827 | `ot_approvals_`（OT承認）。名前キーで削除の口が無いため、既存の `mergeMapByTime` を流用（登録1行のみ） |
| v828 | `shiftopt_`（シフト最適化の店舗ルール・個人の制約）。同上、登録1行のみ |
| v829 | `shift_fixed_`（固定シフト）。**人ごとに個別保存**へ変更。保存時刻・保存者を記録し、消去も記録として残す。汎用の `mergeMapStamped` / `_coversMapStamped` を新設 |
| v830 | `orders_`（発注）。**1件ごとに保存**へ変更。取消と「明細を全部消した下書き」を論理削除に。読み書きを `getOrders` / `getOrdersRaw` / `saveOrders` / `deleteOrder` に集約（キーを直接触る場所をゼロに） |
| v831 | `help_requests_`（ヘルプ募集）。募集は1件ごと、**志願は人ごと**に時刻と名前で記録。取り下げも記録として残す。`volunteers` は表示用に作り直すので画面側は変更なし |
| v832 | `warnings_`（注意ログ）。合流ルール（追記のみ・和集合）に加え、**容量整理の対象から外した**。`lsIsPrunable` が「実データから再計算される」と誤分類していた |
| v833 | `dailyops_`（テンプレート・当日のチェック状態・完了記録の3つ）。項目ごとに保存。テンプレートの削除を論理削除に。`getDailyOps` が保存時刻を引き継ぐよう修正（教訓⑯） |
| v834 | `menu_mix_`（出数）。**メニュー1品ごと**に変わった時刻(`_im`)を残す。0 にした操作も時刻として残すので、古い写しから出数が戻らない |
| v835 | `reminded_`（Remind済み）。名前の和集合。`remindAll` で置き換えても、すでに送った人を落とさない |
...
**これは塞ぐ対象ではない。死んでいる。**
`saveLaborHour` / `doToastLaborFetch` のどちらにも呼び出し元が無く、
勤怠画面は `laborCoreForYm()`（`getTipLabor` 由来）で描いている。
選択肢は3つ。

1. `OP_SYNC_PREFIX` から外す（同期しない。localStorage には残る）
2. 機能ごと消す（`getLaborHours` / `setLaborHours` / `saveLaborHour` /
   `openLaborHourModal` / `doToastLaborFetch` を削除）
3. 画面から呼べるように戻す（勤怠の手入力を復活させたい場合）

**どれにするか Moto さんに確認してから着手する。**
1と2は実害ゼロだが、3を選ぶなら手入力データになるので合流ルールと保護が要る。

**下調べは全部済ませてある。** 次はこの表から選ぶ。

| キー | 生死 | 形 | 物理削除 | 備考 |
|---|---|---|---|---|
| `reminded_` | 生 | 配列 | **あり** | 同上 |
| `labor_hours_` | **死** | `{従業員名:{regular,ot,source}}` | — | **書き込み関数がどこからも呼ばれていない**（`saveLaborHour` / `doToastLaborFetch` に呼び出し元が無い）。画面は `laborCoreForYm()`（`getTipLabor` 由来）で描いている。塞ぐ価値なし。`OP_SYNC_PREFIX` から外すか、機能ごと消すかを Moto さんに確認 |

**物理削除があるキーに和集合の合流ルールを付けてはいけない。**
消したはずの行がマージで復活する（`SPLIT_MERGE_SAFE_KEYS` のコメントに書いてあるとおり）。
先に論理削除（`_deleted`）へ変え、読み取り側で除外するところまでやってから登録する。

**残り1個。塞ぐ作業としては完了。**

**クラウド同期の対象で、丸ごと上書きされるキーは無くなった。**
残る `labor_hours_` は死んでいるので、塞ぐのではなく扱いを決める話（上記）。
- `reminded_`（リマインド済み）— 実害が小さいので後回しでよい
- `menu_mix_`（出数）— `savedAt` で記録ごとに新しい方を採る。項目の和集合にはしない
- `labor_hours_` は死んでいるので、塞ぐのではなく `OP_SYNC_PREFIX` から外すか機能ごと消すかの判断が要る
...
**`fc_monthly_` は v826、`ot_approvals_` は v827、`shiftopt_` は v828 で対応済み。**
`_stDef()` を使う案は**間違いだった**ので注意（第5章⑭）。
包まれていない値では効かず、監査だけ通って実態は丸ごと上書きのままになる。
記録そのものに `_at` を持たせ、項目ごとに合流する方式にした。

**次の候補**は `labor_hours_`（名前キーなので既存の `mergeMapByTime` がそのまま使える見込み）か、
配列系の `orders_` / `help_requests_`（複数人が触るため消えると痛い）。
いずれも**中身の形を確認してから**登録すること。
...
| `fc_monthly_<店>_<ym>` | **単一オブジェクト** `{beginInv,endInv,sales,beginAuto,history[]}`（v826で登録済み） |
...
`menu_mix_` は「無い＝0」なので、1品ごとの時刻で消したかどうかを判定する形にした。
最初の実装は「時刻が同点ならローカル」だけを見ており、
**どちらにも印が無い v833 以前の記録どうしを合流すると、
ローカルに入っていないメニューを『消した』とみなして落としていた。**

印が無いのは「まだ一度も保存していない」だけで、消した証拠ではない。
**両方に印が無いときは、入っているほうを残す。**
これも v834 の検証で「印の無い記録どうし」の試験を書いて初めて出た。

### ⑯ 「作り直して保存する」画面は、保存時刻を落として全部を上書きに戻す

`getDailyOps()` は毎回テンプレートから当日の一覧を作り直す。
そのとき `Object.assign({}, t, {done, by})` としており、
**前回保存したときの印(`_mut`/`_by`)が落ちていた**。
結果、保存のたびに全項目の時刻が進み、1項目ごとに保存しているつもりで
「あとに保存したほうが全部勝つ」に戻っていた。

**登録して終わりにせず、実際に2台ぶんの保存を通して確かめること。**
これは v833 の検証で 2人が別の項目にチェックする試験を書いて初めて出た。
`if(e && e._mut!=null) o._mut = e._mut;` で引き継ぐようにして解決。

**作り直し系の画面（テンプレートから生成／再計算して保存）は必ず疑う。**

### ⑮ 「再計算できる」の分類には、書いた人の思い込みが残っている

`lsIsPrunable` の `_REGENERABLE` に `warnings_` が
「実データから再計算される」というコメント付きで入っていた（v832 で削除）。
実際には確認ダイアログを経て「誰が・いつ・何について」を残す**手入力の記録**で、再計算できない。
容量が足りなくなると端末から丸ごと消え、そのあと1件記録しただけで
クラウド側の注意ログが全部その1件に置き換わっていた。しかも**誰も気づけない**。

⑩「再生成できる」は「自動で戻る」ではない、の続き。
**合流ルールを足すときは、そのキーが容量整理でどう扱われているかも必ず一緒に見る。**
`lsIsPrunable` / `_lsFreeOldLaborKeys` / `LS_NEVER_FREE` の3か所ある。

### ⑭ 「形が合う合流関数」と「実際に効く合流関数」は別

`fc_monthly_` に v750 の `_stDef()` を当てようとしたが、使えなかった。
`_stDef()`/`mergeStamped` は**印で包まれた値** `{__st:1, v:…, _at, _by}` を前提にしており、
包んでいるのは `_splitWrite` の obj 型分割キーだけ（index.html 7453行）。
`fc_monthly_` は分割キーではないので誰も包まず、両側とも `_stAt()=0` →
常にローカルが勝つ＝**丸ごと上書きと同じ挙動のまま、監査だけが OK になる**。

⑬と同じ形の失敗。**「登録した」と「守られている」は別。**
合流関数を選ぶときは、**その関数が読む印を、誰がいつ書いているかまで確かめる。**
...
### 複数端末での保存（v829・Motoさん指示・確定）

- **同じ入れ物へ複数の人が別々の相手（従業員など）を入力する画面は、相手ごとに個別に保存する。**
  「全員ぶんの表を丸ごと書き戻す」作りにしない
- **基本はクラウドに保存し、ぶつかったら保存日時で上書き（あとから保存したほうが勝つ）**
- **保存者の名前を必ず記録する**（誰の保存が勝ったか追える）
- **消去も1つの保存として残す**（`_deleted`）。物理削除にすると、消す前の写しを持った端末から復活する
- 実装は `_stWrap()` で包み（`{__st:1, v:中身, _at, _by}`）、合流は `mergeMapStamped` を使う。
  読み取り関数の側で包みを外し、消去済みを除くので、**画面のコードは変えなくてよい**

**名前キーの入れ物（v829）**：`_stWrap()` で包み、合流は `mergeMapStamped`。
**id を持つ配列（v830）**：`_stampListBy()` で `_mut`（保存時刻）を進め、`_by` を付ける。
合流は既存の `mergeStampedList`。削除は `_deleted` を立てるだけにする。

**粒度は「誰が同時に触るか」で決める（v831）**：
`help_requests_` は募集1件ごとで足りると思ったが、**志願者は複数の人が同時に手を挙げる**。
募集を1件まるごと新しい保存で上書きすると、同じ頃に別端末で挙げた人が消える。
配列の中の小さな一覧（志願者・確定者）まで見て、
**取り消しの口があるものは人ごとに時刻で判定、追記だけのものは和集合**にする。

どちらも **読み書きをアクセサ関数に集約し、キーを直接触る場所をゼロにする**。
`getXxx()`＝画面向け（削除済みを除く）／ `getXxxRaw()`＝書き換え用／ `saveXxx()`＝保存の出口。
これをやらないと、あとから足した画面が素の `ls()` を呼んで保護をすり抜ける。
...
| `verify_v762`〜`v835.js` | 各版の検証 |
| `index_v782`〜`v835_backup.html` | 検証が「修正前」として使う |
