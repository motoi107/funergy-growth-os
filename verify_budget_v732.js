/* v732: 月別ステータス／過去月の保護／月の紐づけ補正 を実ファイルの関数で検証 */
const fs = require('fs');
const vm = require('vm');
...
const names = ['budYmStr', 'budMon', '_budLegacyYm', 'budStatusFor', 'budSetStatus', 'budMonMeta',
  'migrateBudgetStatusYm', 'migrateBudgetStatusYmFix', '_budGuessStatusYm', '_ymFromStamp', 'budPinLegacyMonth', 'canEditBudget', '_splitPrefix', '_splitRead', '_splitWrite',
...
console.log('\n== 10) 【v731】実機で起きた紐づけずれ：Aiea / Piikoi 型 ==');
{
  /* 7/5 に提出・承認されたが、period は作成時の '2026.6' のまま＝月データなし */
  for (const k of Object.keys(store)) delete store[k];
  localStorage.setItem('spl_budgets_v2_F04-A', JSON.stringify([{
    storeId: 'F04-A', period: '2026.6', status: '承認済み',
    submittedBy: 'Yuki', submittedAt: '2026/07/05 11:52',
    approvedBy: 'Moto', approvedAt: '2026/07/05 14:10',
    dow: { lunch: [{ guests: 30, spend: 20 }], dinner: [], takeout: [] }
  }]));
  const b0 = S.ls('budgets_v2', [])[0];
  eq('承認日(7/5)から7月と判定される', S._budGuessStatusYm(b0), '2026-07');
  eq('日付書式の解析', [S._ymFromStamp('2026/07/05 14:10'), S._ymFromStamp('2026-07-05T00:00:00Z'), S._ymFromStamp('')], ['2026-07','2026-07','']);

  localStorage.removeItem('_bud_status_ym_v729');
  localStorage.removeItem('_bud_status_ym_fix_v731');
  S.migrateBudgetStatusYm();
  const b1 = S.ls('budgets_v2', []).find(x => x.storeId === 'F04-A');
  eq('7月が承認済みになる（6月ではない）', S.budStatusFor(b1, '2026-07'), '承認済み');
  eq('6月は下書き', S.budStatusFor(b1, '2026-06'), '下書き');
  YM = { y: 2026, m: 8 };
  eq('8月は下書き＝編集できる', S.budStatusFor(b1, '2026-08'), '下書き');
}

console.log('\n== 11) 【v731】v729で6月に紐づいてしまったものを補正する ==');
{
  for (const k of Object.keys(store)) delete store[k];
  /* v729 が period から '2026-06' を付けてしまった状態を再現 */
  localStorage.setItem('spl_budgets_v2_F04-P', JSON.stringify([{
    storeId: 'F04-P', period: '2026.6', status: '承認済み', _statusYm: '2026-06',
    submittedBy: 'Yuki', submittedAt: '2026/07/05 11:48',
    approvedBy: 'Moto', approvedAt: '2026/07/05 14:12',
    dow: { lunch: [{ guests: 25, spend: 19 }], dinner: [], takeout: [] }
  }]));
  localStorage.removeItem('_bud_status_ym_fix_v731');
  const before = S.ls('budgets_v2', []).find(x => x.storeId === 'F04-P');
  eq('補正前は6月に紐づいている', S.budStatusFor(before, '2026-06'), '承認済み');
  S.migrateBudgetStatusYmFix();
  const after = S.ls('budgets_v2', []).find(x => x.storeId === 'F04-P');
  eq('補正後は7月が承認済み', S.budStatusFor(after, '2026-07'), '承認済み');
  eq('6月は下書きに戻る', S.budStatusFor(after, '2026-06'), '下書き');
  eq('曜日設定などの数字は動かしていない', after.dow.lunch[0].guests, 25);
}

console.log('\n== 12) 【v731】補正が安全側に倒れること ==');
{
  /* 月データがある月は動かさない */
  for (const k of Object.keys(store)) delete store[k];
  localStorage.setItem('spl_budgets_v2_F01', JSON.stringify([{
    storeId: 'F01', period: '2026.6', status: '承認済み', _statusYm: '2026-06',
    approvedAt: '2026/07/05 14:00',
    months: { '2026-06': { _ts: 1, dow: { lunch: [{ guests: 10, spend: 10 }], dinner: [], takeout: [] } } }
  }]));
  localStorage.removeItem('_bud_status_ym_fix_v731');
  S.migrateBudgetStatusYmFix();
  const b = S.ls('budgets_v2', []).find(x => x.storeId === 'F01');
  eq('実データがある月は動かさない', b._statusYm, '2026-06');
  eq('6月は承認済みのまま', S.budStatusFor(b, '2026-06'), '承認済み');

  /* 移動先に実データがある場合も動かさない */
  for (const k of Object.keys(store)) delete store[k];
  localStorage.setItem('spl_budgets_v2_F02', JSON.stringify([{
    storeId: 'F02', period: '2026.6', status: '承認済み', _statusYm: '2026-06',
    approvedAt: '2026/07/05 14:00',
    months: { '2026-07': { _ts: 1, status: '下書き', dow: { lunch: [{ guests: 9, spend: 9 }], dinner: [], takeout: [] } } }
  }]));
  localStorage.removeItem('_bud_status_ym_fix_v731');
  S.migrateBudgetStatusYmFix();
  const b2 = S.ls('budgets_v2', []).find(x => x.storeId === 'F02');
  eq('移動先に実データがあれば触らない', b2._statusYm, '2026-06');
  eq('7月の状態は上書きされない', S.budStatusFor(b2, '2026-07'), '下書き');
}


console.log('\n== 13) 【v732】v730で誤った月に固定してしまった場合の救済 ==');
{
  /* 店舗端末が v730 のまま8月を保存し、7月の数字が「6月」として固定された状態を再現 */
  for (const k of Object.keys(store)) delete store[k];
  localStorage.setItem('spl_budgets_v2_F04-P', JSON.stringify([{
    storeId: 'F04-P', period: '2026.6', status: '承認済み', _statusYm: '2026-06',
    submittedBy: 'Yuki', submittedAt: '2026/07/05 11:48',
    approvedBy: 'Moto', approvedAt: '2026/07/05 14:12',
    dow: { lunch: [{ guests: 88, spend: 30 }], dinner: [], takeout: [] },   /* 8月の内容で上書き済み */
    months: {
      '2026-06': { _ts: 1, _pinned: true, status: '承認済み', approvedBy: 'Moto', approvedAt: '2026/07/05 14:12',
                   dow: { lunch: [{ guests: 25, spend: 19 }], dinner: [], takeout: [] } },   /* 実体は7月の数字 */
      '2026-08': { _ts: 2, dow: { lunch: [{ guests: 88, spend: 30 }], dinner: [], takeout: [] } }
    }
  }]));
  localStorage.removeItem('_bud_status_ym_fix_v731');
  const before = S.ls('budgets_v2', []).find(x => x.storeId === 'F04-P');
  eq('補正前は6月に固定されている', S.budStatusFor(before, '2026-06'), '承認済み');
  S.migrateBudgetStatusYmFix();
  const after = S.ls('budgets_v2', []).find(x => x.storeId === 'F04-P');
  eq('7月へ移る', S.budStatusFor(after, '2026-07'), '承認済み');
  eq('7月の数字（25）も一緒に移る', after.months['2026-07'].dow.lunch[0].guests, 25);
  eq('6月は空になる', !!after.months['2026-06'], false);
  eq('8月の数字は無傷', after.months['2026-08'].dow.lunch[0].guests, 88);
  eq('承認者も維持', S.budMonMeta(after, '2026-07').approvedBy, 'Moto');
}

console.log('\n== 14) 【v732】手入力の月データは絶対に動かさない ==');
{
  for (const k of Object.keys(store)) delete store[k];
  localStorage.setItem('spl_budgets_v2_F01', JSON.stringify([{
    storeId: 'F01', period: '2026.6', status: '承認済み', _statusYm: '2026-06',
    approvedAt: '2026/07/05 14:00',
    months: { '2026-06': { _ts: 1, status: '承認済み', dow: { lunch: [{ guests: 10, spend: 10 }], dinner: [], takeout: [] } } }
  }]));
  localStorage.removeItem('_bud_status_ym_fix_v731');
  S.migrateBudgetStatusYmFix();
  const b = S.ls('budgets_v2', []).find(x => x.storeId === 'F01');
  eq('_pinned が無い月は動かさない', b._statusYm, '2026-06');
  eq('6月の数字はそのまま', b.months['2026-06'].dow.lunch[0].guests, 10);
  eq('7月は作られない', !!(b.months && b.months['2026-07']), false);
}
...
console.log('\n== 15) 【v732】固定した月には印が付く ==');
{
  ...
  S.migrateBudgetStatusYm();
  const b = S.ls('budgets_v2', []).find(x => x.storeId === 'F04-A');
  eq('承認日から7月に紐づく', b._statusYm, '2026-07');
  YM = { y: 2026, m: 8 };
  S.budPinLegacyMonth(b, '2026-08');
  eq('固定した7月に印が付く', b.months['2026-07']._pinned, true);
  eq('7月の数字が保存される', b.months['2026-07'].dow.lunch[0].guests, 30);
  eq('7月は承認済みのまま', S.budStatusFor(b, '2026-07'), '承認済み');
}
