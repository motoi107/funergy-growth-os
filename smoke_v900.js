...
const NEED = ['DISCOUNT_FORM_BASE','DISCOUNT_E_STORE','DISCOUNT_E_HOME','SURVEY_STORE_NAME',
  'SURVEY_HOME_LABEL','SURVEY_OTHER_LABEL',
...
chk('ログイン後の着地で QR コードを拾う', () =>
  /dcPickStoreFromUrl\(\)\)\{[\s\S]{0,80}curPage='discount'/.test(code)
  || 'QRから開いても割引画面に着地しない');

/* v897 の実機バグ：中身は割引証なのにタブは「ダッシュボード」のままだった。
   buildNav() は着地処理より前に走っているので、行き先を変えたら組み直す必要がある。 */
chk('着地で行き先を変えたらナビを組み直す', () => {
  const m = code.match(/curPage='discount'; curGroup=groupOfPage\('discount'\)\.id; buildNav\(\);/);
  return !!m || 'ナビのハイライトが前の画面のまま残る';
});
...
const ctx = vm.createContext({
  ...
  fetch: async (url, opt) => { posted = { url, body: JSON.parse(opt.body) };
    return { ok: true, json: async () => ({ ok: true, is_home_store: false }) }; },
});
...
/* 部分一致だと「他店」が「他店 / Another store」に前方一致してしまい、
   併記漏れを見逃す。末尾までの完全一致で見る。 */
const encHome = encodeURIComponent(A.SURVEY_HOME_LABEL);
const encOther = encodeURIComponent(A.SURVEY_OTHER_LABEL);

chk('自店/他店の文言が日英併記になっている', () => {
  const jaEn = (s) => /[぀-ヿ一-鿿]/.test(s) && /[A-Za-z]{2,}/.test(s);
  if (!jaEn(A.SURVEY_HOME_LABEL)) return '自店が併記でない: ' + A.SURVEY_HOME_LABEL;
  if (!jaEn(A.SURVEY_OTHER_LABEL)) return '他店が併記でない: ' + A.SURVEY_OTHER_LABEL;
  return true;
});

chk('他店のとき他店ラベルで事前入力される（完全一致）', () => {
  const u = A.surveyUrl('F04-K', 'F01');
  const want = 'entry.1145182546=' + encOther;
  if (u.indexOf('entry.2100670339=' + encodeURIComponent('Totoya Kaimuki')) < 0) return '店舗が違う: ' + u;
  return u.endsWith(want) || ('末尾が一致しない: ' + u);
});

chk('自店のとき自店ラベルで事前入力される（完全一致）', () =>
  A.surveyUrl('F01', 'F01').endsWith('entry.1145182546=' + encHome) || A.surveyUrl('F01','F01'));

chk('自店と他店のラベルが別物である', () =>
  A.SURVEY_HOME_LABEL !== A.SURVEY_OTHER_LABEL || '同じ文字列になっている');
chk('未知の店舗コードは null（空欄送信を防ぐ）', () =>
  A.surveyUrl('F99', 'F01') === null || 'null を返していない');

chk('dcSelfCheck に自店/他店も出る', () => {
  const s = A.dcSelfCheck();
  return (s.indexOf(A.SURVEY_HOME_LABEL) >= 0 && s.indexOf(A.SURVEY_OTHER_LABEL) >= 0)
    || '照合用の一覧に出ない';
});
...
(async () => {
  ...
  chk('Edge Function へ送っている（anon で直接テーブルに書かない）', () => {
    if (!posted) return 'POST されていない';
    if (posted.url.indexOf('/functions/v1/discount-log') < 0) return 'URL: ' + posted.url;
    if (posted.url.indexOf('/rest/v1/') >= 0) return 'REST に直接書いている';
    return true;
  });

  chk('送るのは本人確認の材料と店舗コードだけ', () => {
    const b = posted.body;
    if (b.action !== 'use') return 'action: ' + b.action;
    if (b.name !== 'Test Crew' || b.pin !== PIN) return '本人確認の材料が無い';
    if (b.store !== 'F04-K') return 'store: ' + b.store;
    /* 行の中身はサーバーが組み立てる。クライアントの申告は送らない。 */
    const leak = ['staff_id','staff_grade','staff_home_store','is_home_store','used_at']
      .filter(k => k in b);
    return leak.length === 0 || ('クライアントが申告している: ' + leak.join(','));
  });

  chk('ソースに staff_discount_use への直接 POST が残っていない', () => {
    const m = code.match(/rest\/v1\/staff_discount_use/g);
    return !m || ('直接書く経路が ' + m.length + ' 件残っている');
  });

  chk('自店/他店はサーバーの判定を使う', () => {
    const u = A.surveyUrl('F01', 'F01', false);   /* 所属店だがサーバーは「他店」と判定 */
    return u.endsWith('entry.1145182546=' + encOther) || 'サーバー判定が効かない: ' + u;
  });
  ...
  ctx.fetch = async () => ({ ok: false, status: 500,
    json: async () => ({ ok:false, error:'insert_failed', detail:'null value violates not-null constraint' }) });
  A._dcState.used = false; A._dcState.busy = false;
  main.innerHTML = A.renderDiscount();
  await A.useDiscount();
    chk('関数が返した detail を画面に出す', () => {
    const err = win.document.getElementById('dc-error');
    return err.textContent.indexOf('violates not-null') >= 0
      || ('detail を捨てている: ' + err.textContent);
  });
