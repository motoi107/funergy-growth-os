/* v723 日次食材差異(簡素版) — 自動テスト（実コードをモック環境で実行） */
const fs = require('fs');
const vm = require('vm');
...
const ctx = {
  ...
  getIngredients: () => [
    { code: 6034, name: 'まぐろ 冷凍ミンチ', unit: 'g', price: 4.95, qty: 454, yield: 100 },
    { code: 6006, name: 'いくら JFC', unit: 'g', price: 38.0, qty: 500, yield: 100 },
  ],
...
vm.runInContext(fs.readFileSync('/home/claude/fv_module_v723.js', 'utf8'), ctx);
...
console.log('== 15) 画面レンダリング(スモーク) と 再描画ループが起きないこと ==');
{
  setupConfig();
  /* 出数キャッシュあり + renderPage スパイ: 描画中に再帰呼び出しが発生しないこと(v722の凍結原因の修正確認) */
  ...
  eq('一覧タブがHTMLを返す', typeof html === 'string' && html.indexOf('日次食材差異') >= 0, true);
  ...
  /* キャッシュ無し: 取得は1セッション1回だけ */
  delete store[ctx.fvSalesKey(sid, ym)];
  ctx.fvEnsureSales(sid, ym);
  ctx.fvEnsureSales(sid, ym);
  eq('キャッシュ無しでも取得は1回だけ(連打・再描画でも増えない)', fetches, 1);
  ctx._fvTab = 'count'; eq('棚卸タブ', ctx.renderFvVariance().indexOf('保存') >= 0, true);
  ctx._fvTab = 'moves'; eq('入出庫タブ', ctx.renderFvVariance().indexOf('登録') >= 0, true);
  ctx._fvTab = 'config'; eq('設定タブ', ctx.renderFvVariance().indexOf('$/g') >= 0 || ctx.renderFvVariance().indexOf('単価未設定') >= 0, true);
  ctx._fvTab = 'list';
  ctx.renderPage=()=>{};
}
...
console.log('== 16) 7日集計(オンデマンド)が例外なく計算される ==');
{
  ...
  ctx.fvOpenSummary(sid, '2026-07-24');
