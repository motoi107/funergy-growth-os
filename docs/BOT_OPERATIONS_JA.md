...
ユーザー指示により、初期一覧は勤怠、通常取得と日次取得は勤怠のみ。商品/モディファイア単位のVoidは新規取込・通常一覧・送信から除外する。既存案件と履歴は削除しない。

後で「会計・決済Void」から明示的に取得できる対象は、注文全体/会計全体のvoided=true、決済paymentStatus=VOIDED。親が取消済みなら子の取消は重ねて案件化しない。Paymentの理由等はvoidInfoから読む。商品取消の積上げを会計取消と推定しない。取得日は従来通り注文の営業日基準で、過去営業日の決済が後日取消された場合の網羅は別途検証が必要。まず勤怠の実運用を確認する。
