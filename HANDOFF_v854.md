...
| | 状態 |
|---|---|
| 現行版 | **v854**。**v849〜v854 が GitHub 未 push**（v848 までは push 済み） |
| 検証 | `PASS=4517 / FAIL=0 / 実行=78/93`。検証93本・バックアップ73件 |
...
| **Motoさん待ち③** | **本部予算**（成長管理→本部予算タブ）で 客数・客単価。KPI管理ぶんは設定済み |
...
```bash
unzip funergy_v854_handoff.zip
tar -xJf handoff_v854.tar.xz
cd handoff_v854
...
```
合計 PASS=4517  FAIL=0  実行=78/93
合流ルールの未定義: 1
同期対象で未保護: 0
予算の対象月: 3 / 7
APP_VERSION=854 / SW_BUILD=854
CRLF=58096 / lone LF=0
```
