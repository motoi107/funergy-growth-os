...
test('labor and finance have separate owners, counts, statuses, unresolved cases without completed cases',()=>{
 ...
 for(const id of ['#1']){assert.match(labor,new RegExp('案件：'+id));assert.doesNotMatch(finance,new RegExp('案件：'+id));}
 for(const id of ['#2','#3']){assert.match(finance,new RegExp('案件：'+id));assert.doesNotMatch(labor,new RegExp('案件：'+id));}
...
test('zeroes and collection failure are evaluated independently for each responsibility',()=>{
 ...
 assert.equal(reports[0].messages.length,0);assert.doesNotMatch(reports[0].text,/決済データ/);
 assert.match(reports[1].text,/決済データの取得が未完了/);assert.doesNotMatch(reports[1].text,/未解決の異常はありません/);
 const ready=formatResponsibleMorningReports({...s,finance_enabled:true,finance_ok:11});assert.ok(ready.reports.every(r=>r.complete));assert.equal(ready.messages.length,0);
});
...
test('missing classification and incomplete snapshots fail explicitly instead of silently omitting cases',()=>{
 assert.throws(()=>formatResponsibleMorningReports({...combined,details:[{code:'#9'}]}),/missing_case_kind/);
