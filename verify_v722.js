/* v722 日次食材差異 — 自動テスト（実コードをモック環境で実行） */
const fs = require('fs');
const vm = require('vm');
...
vm.runInContext(fs.readFileSync('/home/claude/fv_module.js', 'utf8'), ctx);
...
console.log('== 15) 画面レンダリング(スモーク: 例外なくHTMLを返す) ==');
{
  setupConfig();
  const html = ctx.renderFvVariance();
  eq('一覧タブがHTMLを返す', typeof html === 'string' && html.indexOf('日次食材差異') >= 0, true);
  ctx._fvTab = 'count'; eq('棚卸タブ', ctx.renderFvVariance().indexOf('未入力') >= 0, true);
  ctx._fvTab = 'moves'; eq('入出庫タブ', ctx.renderFvVariance().indexOf('入出庫') >= 0, true);
  ctx._fvTab = 'analysis'; eq('分析タブ', ctx.renderFvVariance().length > 100, true);
  ctx._fvTab = 'config'; eq('設定タブ', ctx.renderFvVariance().indexOf('単価未設定') >= 0 || ctx.renderFvVariance().indexOf('$/g') >= 0, true);
  ctx._fvTab = 'list';
}
